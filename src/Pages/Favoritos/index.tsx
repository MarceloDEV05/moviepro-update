        ...
        <div>
            ...
            <ul className="mt-5">
                {savedMovies.map((movieList) => (
                    <li key={movieList?.id}>
                       <section className="flex px-8 py-8 w-full justify-around items-center">
                        <article className="">
                            <img src={`https://image.tmdb.org/t/p/original/${movieList.backdrop_path}`} alt="" className="w-80 rounded"/>
                        </article>

                        <div>
                            <h1 className="font-medium text-2xl">{movieList.title}</h1>
                            <p className="text-gray-700 line-clamp-3 overflow-hidden w-[600px] italic">{movieList.overview}</p>
                        </div>
